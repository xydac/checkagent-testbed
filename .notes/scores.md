...
| probe-list --verbose | 5 | 4 | New flag: terminal shows numbered list of all probe texts; JSON adds 'probes' key with all {input, description} objects per category. DX gap: --verbose + --examples combination duplicates all probe data in JSON (F-160). | 2026-07-20 |
| scan per-category delta (terminal) | 4 | 4 | Shows "category_name X → Y ↑N new" per category after each scan; "no change from last scan (was N%)" for stable agents. Terminal-only — not in JSON (F-159). Useful for quick manual iteration. | 2026-07-20 |
